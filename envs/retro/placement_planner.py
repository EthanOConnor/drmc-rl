...
class SpawnReachability:
    ...
    action_to_terminal_node: Dict[int, int]  # macro action -> reach.nodes index (locked)


class PlacementPlanner:
    """Compute feasibility masks and reconstruct minimal-time scripts."""

    def __init__(self, *, max_frames: int = 2048) -> None:
    ...
    def plan_action(self, spawn: SpawnReachability, action: int) -> Optional[PlanResult]:
        ...
        action_i = int(action)
        node_idx = spawn.action_to_terminal_node.get(action_i)
        if node_idx is None:
            return None
        node = spawn.reach.nodes[int(node_idx)]
        if not node.state.locked:
            return None

        frame_actions = reconstruct_actions(spawn.reach, int(node_idx))
