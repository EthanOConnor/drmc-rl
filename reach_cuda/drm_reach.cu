    ...
    if (tid == 0) {
        ...
        if (sh->spawn_ok) {
            ...
            for (int pose = 0; pose < N_POSES; ++pose) {
                if (ws->wanted[pose]) ws->wanted_ids[nw++] = (u16)pose;
            }
            sh->n_wanted = nw;
        } else {
...
__device__ void run_phase3(
    ...
    for (int depth = 0; depth < max_frames; ++depth) {
        ...
        for (int t = tid; t < cur_n; t += blockDim.x) {
            expand_key(&sh->fm, ws, s3, ws->frontier[cur_buf][t], acts, n_act,
                       p_cur, cur_buf, next_depth,
                       thr_keep_lo, thr_keep_hi, thr, two_words, out_costs);
        }
        __syncthreads();
