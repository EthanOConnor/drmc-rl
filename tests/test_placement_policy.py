...
class TestColorInvariance:
    ...
    def test_ordered_embedding_is_not_symmetric(self):
        """Ordered pair embedding preserves half identity for mixed colors."""
        embedder = OrderedPairEmbedding(num_colors=3, output_dim=9)
        with torch.no_grad():
            embedder.emb.weight.copy_(torch.eye(9))

        colors_1 = torch.tensor([[0, 1]])
        colors_2 = torch.tensor([[1, 0]])

        emb_1 = embedder(colors_1)
        emb_2 = embedder(colors_2)
        assert not torch.allclose(emb_1, emb_2, atol=1e-6)

    def test_policy_can_use_ordered_embedding(self):
        net = PlacementPolicyNet(
            in_channels=12, head_type="dense", pill_embed_type="ordered_onehot"
        )
