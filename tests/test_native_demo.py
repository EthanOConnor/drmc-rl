...
from drmc_rl.native.shm import SHM_SIZE, DrMarioStatePy, open_shared_memory, repo_root
...
def test_demo_reset_matches_disassembly(engine_proc, demo_data):
    ...
    while time.monotonic() < ready_deadline and not (int(state.control_flags) & 0x02):
        time.sleep(0.001)
