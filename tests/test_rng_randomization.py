...
def test_does_not_mark_clear_before_gameplay_detected():
    env, stub = _make_env(auto_start=True)
    env._ram_offsets = {"viruses": {"remaining_addr": f"{0x0400:04X}"}}
    stub.ram[:] = 0
    ...
    _, reward, terminated, truncated, info = env.step(int(Action.NOOP))

    assert not terminated
    assert not truncated
    assert info.get("cleared") is False
    assert reward < env.reward_cfg.terminal_clear_bonus
