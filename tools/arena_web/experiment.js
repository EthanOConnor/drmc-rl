...
function experimentView(data) {
  ...
  experimentPanel.innerHTML = `
    <div class="experiment-heading"><p class="eyebrow">ACTIVE EXPERIMENT</p><span class="badge">${esc(data.status || 'Preparing')}</span></div>
    <h2>${esc(data.title)}</h2><p>${esc(data.hypothesis)}</p>
    <div class="current-work"><span>WORKING ON</span><strong>${esc(data.current_work)}</strong></div>
  ...
  resultsPanel.innerHTML = `
    ...
    ${training.status ? `<div class="section-title"><h3>Pace-conditioned training</h3><p>${esc(training.status)}</p></div>
      ...
      <div><span>Collecting · ${esc(training.current_pace || 'Starting')}</span><strong>${safeNumber(training.collecting_games)} / ${safeNumber(training.collecting_target)} games</strong><small>L${training.current_level || 14} HI · ${safeNumber(training.updates)} updates complete${training.error ? ` · ${esc(training.error)}` : ''}</small></div></div>
