...
class HumanBackend:
    ...
    def _candidates(self, state: Mapping[str, Any]):
        ...
        return planes, opponent_planes, pill, preview, speed, speed_ups, reach, packed

    def _infer(self, request: Mapping[str, Any]) -> dict[str, Any]:
        state = request["state"]
        rating = float(request["target_rating"])
        temperature = float(request.get("temperature", 1.0))
        planes, opponent_planes, pill, preview, speed, speed_ups, reach, packed = self._candidates(
            state
        )
        logits, resolved_rating, rating_clamped = self.runtime.score(
            board_planes=planes,
        ...
        slot = self.runtime.choose(logits, packed.mask, temperature=temperature)
        action = int(packed.actions[slot])
        if pill[0] == pill[1]:
        ...
        timing = self.runtime.timing_prediction(
            ...
            chosen_cost=float(packed.cost[slot]),
            speed=speed,
            speed_ups=speed_ups,
