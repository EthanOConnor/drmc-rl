...
def settings_requirements(settings: dict) -> set[str]:
    """Capabilities an entrant's decision-changing settings need (derived from its knob list)."""
    from drmc_rl.style import knobs
    return knobs.requirements((settings or {}).get("knobs"))


def validate_knob_settings(settings: dict) -> None:
    """Knob lists must name registered knobs with finite lambdas and inline or bundled models."""
    from drmc_rl.style import knobs
    stray = [k for k in (settings or {}) if k.startswith("showy_")]
    if stray:
        raise ValueError(f"settings {stray} are not supported; use knobs: [{{id, version, lambda, model}}]")
    knobs.validate_knobs((settings or {}).get("knobs"))
...
def runtime_capabilities(repo) -> set[str]:
    ...
    if (repo / "drmc_rl" / "style" / "knobs.py").is_file() and arena.is_file() and "knobs.apply" in arena.read_text():
        from drmc_rl.style import knobs
        caps |= knobs.capabilities()      # knob:<id>@<version> for every registered knob
    return caps
