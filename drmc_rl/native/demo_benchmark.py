...
from drmc_rl.native.shm import DrMarioStatePy, open_shared_memory
...
def _default_demo_inputs_path() -> Path:
    env_path = os.environ.get("DRMARIO_DEMO_INPUTS_PATH")
    if env_path:
        return Path(env_path).expanduser()

    rel_path = Path("data") / "drmario_data_demo_inputs.asm"
    candidates = [
        parent / "disassembly" / "dr-mario-disassembly" / rel_path
        for parent in (THIS_DIR, *THIS_DIR.parents)
    ]
    candidates.extend(
        [
            THIS_DIR.parent / "dr-mario-disassembly" / rel_path,
            THIS_DIR / "dr-mario-disassembly" / rel_path,
            THIS_DIR.parent / "drmc-rl" / "dr-mario-disassembly" / rel_path,
        ]
    )
...
DEMO_INPUTS_PATH = _default_demo_inputs_path()
...
def run_benchmark(engine_path: Path, shm_file: Path | None, demo_inputs_path: Path) -> None:
    ...
    try:
        instructions = parse_demo_instruction_set(demo_inputs_path)
...
def main() -> None:
    ...
    parser.add_argument(
        "--engine",
        type=Path,
        default=THIS_DIR / "drmario_engine",
        help="Path to compiled drmario_engine binary.",
    )
    parser.add_argument(
        "--demo-inputs",
        type=Path,
        default=DEMO_INPUTS_PATH,
        help="Path to drmario_data_demo_inputs.asm.",
    )
    parser.add_argument(
        "--shm-file",
        type=Path,
    ...
    if not args.demo_inputs.is_file():
        raise SystemExit(
            "Demo inputs file not found. Pass --demo-inputs or set DRMARIO_DEMO_INPUTS_PATH."
        )

    run_benchmark(engine_exec, args.shm_file, args.demo_inputs.resolve())
