...
The physics core is in `drmc_rl.planning.fast_reach` and mirrors the ROM routines:
...
class PlacementPlanner:
    ...
    def _native_available(self) -> bool:
        if self._reach_backend == "python":
            return False
        try:
            from drmc_rl.planning import native_reach

            return native_reach.is_library_present()
        except Exception:
            return False
...
try:  # pragma: no cover - optional acceleration
    from drmc_rl.planning.native_reach import NativeReachability, NativeReachabilityRunner
