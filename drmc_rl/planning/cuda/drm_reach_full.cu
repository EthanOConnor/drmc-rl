...
// Host mirrors this to size a slot (256-byte units).
extern "C" __global__ void drm_reach_full_slot_bytes(u32 nkeys, u32 fr_cap, u64* out) {
    if (threadIdx.x == 0 && blockIdx.x == 0) {
        u64 bytes = (u64)nkeys * (32u + 4u + 4u + 4u + 16u + 4u + 1u) + (u64)fr_cap * 5u;
        out[0] = bytes;
    }
}
...
// ===========================================================================
// Paced stage 1: drm_reach_bfs_paced's reaction prefix and per-pose simple
// routes, byte-exact. The CPU returns right after this stage when every
// wanted pose has a simple route, or after the exact unrestricted (v4)
// feasibility filter when the remaining wanted poses are unreachable anyway.
// The host resolves the filter with the v4 CUDA costs; every other instance
// (tuck waypoints, bounded probe, exhaustive layered search) stays on the CPU.
// ===========================================================================
#define ROUTE_CAP 1024
#define ST_PACED_UNRESOLVED 32  // some wanted pose has no simple route
#define ST_PACED_ROUTE_CAP 64   // lock horizon exceeds the route scratch

struct PState { int x, y, rot, sc, hv, hd, p, rh, locked; };

// drm_reach_full.c::v4_step
__device__ void pv_step(const FitMask* fm, int thr, PState* s, int act) {
    const int dir = (int)ACT_HOLD_DIR[act];
    const int dn = (int)ACT_HOLD_DOWN[act];
    const int rotation = (int)ACT_ROT[act];
    const int hold_left = dir == HOLD_LEFT, hold_right = dir == HOLD_RIGHT;
    const int press_lr = (hold_left && s->hd != HOLD_LEFT) || (hold_right && s->hd != HOLD_RIGHT);
    ...
    if (drop) {
        ...
        if (ny >= GRID_H || !fits_masked(fm, s->x, ny, s->rot)) {
            s->locked = 1; s->hd = dir; s->rh = rotation; s->p ^= 1;
            return;
        }
        s->y = ny;
    }
    ...
    if (allow_move && dir != HOLD_NEUTRAL && (hold_right ? s->x < 6 + (s->rot & 1) : s->x > 0)) {
        const int nx = s->x + (hold_right ? 1 : -1);
        if (fits_masked(fm, nx, s->y, s->rot)) s->x = nx;
        else s->hv = HOR_BLOCKED;
    }
    if (rotation != 0 && rotation != s->rh) {
        int rx = s->x, rrot = s->rot;
        apply_rotation_masked(fm, &rx, s->y, &rrot, rotation, hold_left);
        s->x = rx; s->rot = rrot;
    }
    s->hd = dir; s->rh = rotation; s->p ^= 1;
}

__device__ inline int paced_button_count(int action) {
    return (ACT_HOLD_DIR[action] != 0) + (ACT_HOLD_DOWN[action] != 0) + (ACT_ROT[action] != 0);
}

// drm_reach_full.c::paced_simple_route without waypoints (switch_y = -1).
__device__ int pv_simple_route(const FitMask* fm, int thr, PState state, int px, int py, int prot,
                               int edge_interval, int motion_interval, int max_buttons, int max_frames,
                               int order, int tap, u8* script) {
    int previous = state.hd * 6 + state.rh, edge_wait = 0, motion_wait = 0;
    for (int frame = 0; frame < max_frames; ++frame) {
        const int dx = px - state.x;
        const int rotations = (state.rot - prot) & 3;
        ...
        if (paced_button_count(action) > max_buttons) return -1;
        PState next = state;
        pv_step(fm, thr, &next, action);
        ...
        edge_wait = changed ? (edge_interval > 0 ? edge_interval - 1 : 0) : (edge_wait ? edge_wait - 1 : 0);
        motion_wait = moved ? (motion_interval > 0 ? motion_interval - 1 : 0) : (motion_wait ? motion_wait - 1 : 0);
        if (frame >= ROUTE_CAP) return -2;
        script[frame] = (u8)action;
...
struct PacedShared {
    ...
    u32 offs[N_POSES];
    PState initial;
    int status, resolved, prefix, max_frames, wanted_count, used;
};

// Instance fields: flags = reaction frames; _pad = edge | motion << 8 | buttons << 16.
extern "C" __global__ void __launch_bounds__(512)
drm_reach_paced_stage1_kernel(
    const Instance* __restrict__ insts, int n, unsigned long long* cursor,
    u8* scratch,                    // slots x 512 poses x 2 x ROUTE_CAP
    int scap,
    u16* __restrict__ out_costs, u16* __restrict__ out_offsets, u16* __restrict__ out_lengths,
    u8* __restrict__ out_scripts, int* __restrict__ out_used, int* __restrict__ out_status,
    u8* __restrict__ out_wanted,    // n x 512: wanted and still unresolved
    u8* __restrict__ out_initial    // n x 8: post-reaction x y rot sc hv hd p rh
) {
    __shared__ PacedShared sh;
    __shared__ int s_idx;
    const int tid = threadIdx.x, T = blockDim.x;
    u8* my_scratch = scratch + (u64)blockIdx.x * N_POSES * 2u * ROUTE_CAP;
    for (;;) {
        if (tid == 0) s_idx = (int)atomicAdd(cursor, 1ull);
        __syncthreads();
        const int idx = s_idx;
        if (idx >= n) return;
        const Instance in = insts[idx];
        const int reaction = (int)in.flags;
        const int edge = (int)(in._pad & 0xFFu), motion = (int)((in._pad >> 8) & 0xFFu);
        const int buttons = (int)((in._pad >> 16) & 0xFFu);
        const int thr = (int)in.thr;
        u16* costs = out_costs + (u64)idx * N_POSES;
        u16* offsets = out_offsets + (u64)idx * N_POSES;
        u16* lengths = out_lengths + (u64)idx * N_POSES;
        u8* wanted_out = out_wanted + (u64)idx * N_POSES;
        u8* script = out_scripts + (u64)idx * (u64)scap;
        for (int p = tid; p < N_POSES; p += T) {
            costs[p] = 0xFFFFu; offsets[p] = 0u; lengths[p] = 0u; wanted_out[p] = 0u; sh.len[p] = 0u;
        }
        if (tid == 0) {
            sh.status = 0; sh.resolved = 0; sh.used = 0; sh.wanted_count = 0;
            const int max_frames = (int)in.max_frames;
            sh.max_frames = max_frames;
            if (max_frames < 1 || max_frames > 65534 || reaction > max_frames || edge > 60 || motion > 60
                || buttons < 1 || buttons > 3 || in.sx >= 8 || in.sy >= 16 || in.srot >= 4
                || thr > 127 || in.sc > thr || in.hd > 2 || in.rh > 2) {
                sh.status = ST_BAD_ARGS;
            } else {
                build_fit_masks(in.cols, &sh.fm);
                if (!fits_masked(&sh.fm, in.sx, in.sy, in.srot)) {
                    sh.resolved = 1;
                } else {
                    PState s = {in.sx, in.sy, in.srot, in.sc, in.hv & 15, in.hd, in.p & 1, in.rh, 0};
                    int prefix = 0;
                    while (prefix < reaction && !s.locked) { pv_step(&sh.fm, thr, &s, 0); ++prefix; }
                    sh.initial = s;
                    sh.prefix = prefix;
                    if (s.locked) {
                        if (prefix > scap) {
                            sh.status = ST_SCRIPT_CAP;
                        } else {
                            const int pose = pose_index(s.x, s.y, s.rot);
                            costs[pose] = lengths[pose] = (u16)prefix;
                            for (int i = 0; i < prefix; ++i) script[i] = 0u;
                            sh.used = prefix;
                            sh.resolved = 1;
                        }
                    } else if (prefix == max_frames) {
                        sh.resolved = 1;
                    } else if (compute_max_lock_frames(s.y, s.sc, thr) + 1 > ROUTE_CAP) {
                        sh.status = ST_PACED_ROUTE_CAP;
                    } else {
                        sh.wanted_count = build_wanted(&sh.fm, s.x, s.y, s.rot, sh.wanted, sh.fl_vis, sh.fl_q);
                    }
                }
            }
        }
        __syncthreads();
        if (sh.status == 0 && !sh.resolved) {
            const int horizon = sh.max_frames - sh.prefix;
            for (int p = tid; p < N_POSES; p += T) {
                if (!sh.wanted[p]) continue;
                u8* work = my_scratch + (u64)p * 2u * ROUTE_CAP;
                u8* best_script = work + ROUTE_CAP;
                int best = horizon + 1;
                for (int tap = 0; tap < 2; ++tap)
                    for (int order = 0; order < 3; ++order) {
                        const int length = pv_simple_route(&sh.fm, thr, sh.initial, p & 7, (p >> 3) & 15,
                            p >> 7, edge, motion, buttons, horizon, order, tap, work);
                        if (length == -2) atomicOr(&sh.status, ST_PACED_ROUTE_CAP);
                        if (length > 0 && length < best) {
                            best = length;
                            for (int i = 0; i < length; ++i) best_script[i] = work[i];
                        }
                    }
                if (best <= horizon) sh.len[p] = (u16)best;
            }
            __syncthreads();
            if (tid == 0 && sh.status == 0) {
                int used = 0, found = 0;
                for (int p = 0; p < N_POSES; ++p) {
                    if (!sh.len[p]) continue;
                    const int length = sh.prefix + sh.len[p];
                    if (used + length > 65535 || used + length > scap) { sh.status = ST_SCRIPT_CAP; break; }
                    sh.offs[p] = (u32)used;
                    used += length;
                    ++found;
                }
                sh.used = used;
                if (found == sh.wanted_count) sh.resolved = 1;
                else sh.status = ST_PACED_UNRESOLVED;
            }
            __syncthreads();
            if (sh.status == 0 || sh.status == ST_PACED_UNRESOLVED) {
                for (int p = tid; p < N_POSES; p += T) {
                    if (sh.wanted[p] && !sh.len[p]) wanted_out[p] = 1u;
                    if (!sh.len[p]) continue;
                    const int length = sh.prefix + sh.len[p];
                    costs[p] = lengths[p] = (u16)length;
                    offsets[p] = (u16)sh.offs[p];
                    u8* dst = script + sh.offs[p];
                    for (int i = 0; i < sh.prefix; ++i) dst[i] = 0u;
                    const u8* src = my_scratch + (u64)p * 2u * ROUTE_CAP + ROUTE_CAP;
                    for (int i = 0; i < sh.len[p]; ++i) dst[sh.prefix + i] = src[i];
                }
            }
        }
        if (tid == 0) {
            out_used[idx] = sh.used;
            out_status[idx] = sh.status;
            u8* init = out_initial + (u64)idx * 8u;
            init[0] = (u8)sh.initial.x; init[1] = (u8)sh.initial.y; init[2] = (u8)sh.initial.rot;
            init[3] = (u8)sh.initial.sc; init[4] = (u8)sh.initial.hv; init[5] = (u8)sh.initial.hd;
            init[6] = (u8)sh.initial.p; init[7] = (u8)sh.initial.rh;
        }
        __syncthreads();
    }
}
