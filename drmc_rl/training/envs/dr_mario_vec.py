...
def make_vec_env(cfg: VecEnvConfig | Dict[str, object] | object) -> DummyVecEnv:
    ...
    if isinstance(cfg, VecEnvConfig):
        ...
        unknown = sorted(set(env_dict) - set(VecEnvConfig.__annotations__))
        if unknown:
            raise ValueError(f"Unknown env configuration keys: {', '.join(unknown)}")
        env_cfg = VecEnvConfig(
            **{
                k: env_dict.get(k, getattr(VecEnvConfig, k))
                for k in VecEnvConfig.__annotations__
            }
        )
...
def _make_real_vec_env(env_cfg: VecEnvConfig, seed: Optional[int] = None) -> Any:
    ...
    def make_env(rank: int):
        def _init():
            ...
            from drmc_rl.envs.libretro.registration import (
                register_env_id,
