...
`drmc_rl/training/algo/*` and `drmc_rl/training/envs/curriculum.py`:
  - reset() -> (obs[N,...], infos[N])
...
def _resolve_reward_config_path() -> Optional[Path]:
    ...
    return (Path(__file__).resolve().parents[2] / "game" / "specs" / "reward_config.json").resolve()
...
class DrMarioPoolVecEnv:
    ...
    def _apply_task_budgets_in_place(
        ...
        *,
        ...
        # `drmc_rl/envs/libretro/placement_env.py::_finalize_step`.
        for i in range(self.num_envs):
            info = self._infos[i]
