...
class DrMarioVsShardedVecEnv:
    """Independent VS pools whose GPU and native phases overlap by shard."""

    def __init__(self, *, num_pairs: int, num_shards: int, **kwargs: Any) -> None:
        shard_count = min(int(num_pairs), max(1, int(num_shards)))
        ...
        try:
            ...
            self.shards = [first]
            opp_cfg = kwargs.get("opponent_pool_cfg")
            for size in sizes[1:]:
                shard_kwargs = dict(kwargs)
                if first._opp_pool is not None:
                    shared_cfg = dict(opp_cfg or {})
                    shared_cfg["pool"] = first._opp_pool
                    shard_kwargs["opponent_pool_cfg"] = shared_cfg
                self.shards.append(DrMarioVsPoolVecEnv(num_pairs=size, **shard_kwargs))
        finally:
            if old_workers is None:
                os.environ.pop("DRMARIO_POOL_WORKERS", None)
            else:
                os.environ["DRMARIO_POOL_WORKERS"] = old_workers

        self.num_pairs = sum(shard.num_pairs for shard in self.shards)
        self.num_envs = sum(shard.num_envs for shard in self.shards)
        ...
        self._executor = ThreadPoolExecutor(
            max_workers=len(self.shards), thread_name_prefix="drmc-vs-shard"
        )

    def reset(self, *, seed: Optional[int] = None, options: Optional[Dict[str, Any]] = None):
        results = [
            shard.reset(seed=None if seed is None else int(seed) + i, options=options)
            for i, shard in enumerate(self.shards)
        ]
        return np.concatenate([result[0] for result in results]), sum(
    ...
    def step(self, actions: Sequence[int]):
        ...
        for shard in self.shards:
            end = offset + shard.num_envs
            futures.append(self._executor.submit(shard.step, actions_arr[offset:end]))
            offset = end
    ...
    def set_attr(self, name: str, values: Any, indices: Optional[Sequence[int]] = None) -> None:
        for shard in self.shards:
            shard.set_attr(name, values, indices)

    def get_vs_metrics(self) -> Dict[str, float]:
        first = self.shards[0]
        metrics = first.get_vs_metrics()
        matches = sum(shard._matches_total for shard in self.shards)
        emu_frames = sum(shard._emu_frames_total for shard in self.shards)
        volleys = sum(shard._volleys_total for shard in self.shards)
        minutes = float(emu_frames) / NES_FPS / 60.0
        cpm = float(volleys) / minutes if minutes > 0 else 0.0
        wins = [value for shard in self.shards for value in shard._win_p1]
        draws = [value for shard in self.shards for value in shard._draws]
        lengths = [value for shard in self.shards for value in shard._match_len_sec]
        metrics.update(
            {
                "vs/matches_total": float(matches),
                "vs/cpm": cpm,
                "vs/cpm_received": cpm,
            }
        )
        if wins:
            metrics["vs/win_rate"] = float(np.mean(wins[-100:]))
        if draws:
            metrics["vs/draw_rate"] = float(np.mean(draws[-100:]))
        if lengths:
            metrics["vs/match_len_p50_sec"] = float(np.median(lengths[-100:]))
        return metrics
    ...
    def maybe_snapshot(
        ...
        *,
        ...
        first = self.shards[0]
        total = sum(shard._matches_since_snapshot for shard in self.shards)
        original = first._matches_since_snapshot
        first._matches_since_snapshot = total
        try:
            saved_lengths = first._match_len_sec
            first._match_len_sec = deque(
                (value for shard in self.shards for value in shard._match_len_sec), maxlen=100
            )
            try:
                created = first.maybe_snapshot(state_dict_getter, cfg=cfg, step=step)
            finally:
                first._match_len_sec = saved_lengths
        finally:
            if first._matches_since_snapshot == total:
                first._matches_since_snapshot = original
        if created:
            for shard in self.shards:
                shard._matches_since_snapshot = 0
        return created

    def skill_games_pending(self) -> int:
        return sum(shard.skill_games_pending() for shard in self.shards)

    def pop_skill_games(self) -> List[Dict[str, float]]:
        return [game for shard in self.shards for game in shard.pop_skill_games()]
...
__all__ = ["DrMarioVsPoolVecEnv", "DrMarioVsShardedVecEnv"]
