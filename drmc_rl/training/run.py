...
from drmc_rl.envs.libretro.core_utils import resolve_libretro_core
from drmc_rl.training.algo.base import AlgoAdapter
from drmc_rl.training.diagnostics.event_bus import EventBus
from drmc_rl.training.diagnostics.logger import DiagLogger
from drmc_rl.training.diagnostics.video import VideoEventHandler
from drmc_rl.training.envs import make_vec_env
from drmc_rl.training.utils.cfg import apply_dot_overrides, load_and_merge_cfg, to_config_node
from drmc_rl.training.utils.reproducibility import pick_device, set_reproducibility, write_environment_file

_CONFIG_DIR = Path(__file__).resolve().parent / "configs"
DEFAULT_BASE_CFG = _CONFIG_DIR / "base.yaml"
DEFAULT_RUN_CFG = _CONFIG_DIR / "smdp_ppo.yaml"
...
Examples:
  ...
  python -m drmc_rl.training.run --ui tui --backend cpp-pool --num_envs 16
  python -m drmc_rl.training.run --ui debug --backend libretro --core quicknes --rom-path legal_ROMs/DrMario.nes --num_envs 1
""",
    )
    parser.add_argument(
        "--algo",
        choices=["ppo_smdp", "smdp_ppo"],
        default=None,
    ...
    parser.add_argument("--env-id", type=str, default=None, help="Gym env id (e.g. DrMarioLibretroEnv-v0)")
    parser.add_argument(
        "--action_space",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Backend: cpp-pool|cpp-vs-pool|libretro|mock",
    )
...
def load_config(args: argparse.Namespace) -> Any:
    ...
    algo = args.algo or cfg_dict.get("algo") or "ppo_smdp"
...
def build_adapter(cfg: Any, env: Any, logger: DiagLogger, event_bus: EventBus, device: str) -> AlgoAdapter:
    if cfg.algo == "ppo_smdp":
        from drmc_rl.training.algo.ppo_smdp import SMDPPPOAdapter as Adapter
...
def main(argv: Any = None) -> None:
    ...
    try:
        if args.ui == "debug":
            from drmc_rl.training.envs.interactive import PlaybackControl, RateLimitedVecEnv, StopTraining
            from drmc_rl.training.ui.runner_debug_tui import RunnerDebugTUI
            ...
            try:
                from drmc_rl.training.ui.event_handler import TUIEventHandler
                from drmc_rl.training.ui.tui import RICH_AVAILABLE, TrainingTUI
