...
def git_commit() -> str:
    ...
    try:
        sha = subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=Path(__file__).resolve().parents[3])
        sha = sha.decode("utf-8").strip()
    except Exception:  # pragma: no cover - git optional in tests
        return "unknown"
    try:
        subprocess.check_call(["git", "diff", "--quiet"], cwd=Path(__file__).resolve().parents[3])
        dirty = False
