...
`DrMarioLibretroEnv` is a per-frame controller environment. This wrapper turns it
...
from drmc_rl.planning.planner import (
    BoardState,
    PillSnapshot,
    PlacementPlanner,
    SpawnReachability,
)
from drmc_rl.planning.placement_space import GRID_HEIGHT, GRID_WIDTH, TOTAL_ACTIONS
from drmc_rl.game.state import DrMarioState
...
class _PerfBreakdown:
    ...
    macro decision. The base env (DrMarioLibretroEnv) emits `perf/env_*_sec` keys
...
class DrMarioPlacementEnv(gym.Wrapper):
    ...
    def _maybe_log_pose_mismatch(
        ...
        *,
        ...
        try:
            from drmc_rl.planning.placement_space import unflatten as unflatten_action
    ...
    def _advance_until_decision_after_reset(
        ...
        obs, out_info, _reward, terminated, truncated = self._advance_until_decision(obs, info)
    ...
    def step(self, action: int):
        ...
        # Execute the per-frame controller script through the emulator.
        obs = self._last_obs
...
def make_placement_env(**kwargs: Any) -> gym.Env:
    ...
    base = DrMarioLibretroEnv(**kwargs)
