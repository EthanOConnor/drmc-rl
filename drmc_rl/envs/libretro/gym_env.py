...
from drmc_rl.envs.backends import make_backend
from drmc_rl.envs.backends.base import NES_BUTTONS
from drmc_rl.game.render import state_to_rgb
from drmc_rl.game.rewards import PotentialShaper
from drmc_rl.game.specs.timeouts import get_level_timeout
from drmc_rl.game.state import DrMarioState, build_state

A_BUTTON_INDEX = NES_BUTTONS.index("A")
B_BUTTON_INDEX = NES_BUTTONS.index("B")
from time import perf_counter

from drmc_rl.envs.libretro.action_adapters import discrete10_to_buttons
...
class RewardConfig:
    ...
    # sync with `drmc_rl/game/specs/reward_config.json`. This ensures that if config
...
class DrMarioLibretroEnv(gym.Env):
    ...
    Observation modes:
      ...
      - state: structured 16x8 board tensor (channel count depends on the
        ...
        :func:`drmc_rl.game.specs.ram_to_state.set_state_representation`), frame-stack 4.
        See docs/STATE_OBS_AND_RAM_MAPPING.md for channel spec and mapping;
        RAM offsets are configured via drmc_rl/game/specs/ram_offsets.json (for this
    ...
    def _resolve_reward_config(
        ...
        specs_dir = Path(__file__).resolve().parents[2] / "game" / "specs"
        candidates.append(("default", specs_dir / "reward_config.json"))
    ...
    def _maybe_patch_speed_setting(self) -> None:
        ...
        Libretro runs through menus where speed is usually "medium" by
        default. Patch the in-game speed-setting mirrors after gameplay begins.
        """
    ...
    def reset(
        ...
        *,
        ...
        # Backends may accept RNG seeds before reset, which is necessary for
        # randomizing the initial virus layout.
        pre_seeded = False
